    ...
    def create_font_window(self):
        with dpg.window(
            ...
            with dpg.child_window(autosize_x=True, height=-120):
                dpg.add_text(
                    "This is the font window. To use it, find your Fonts folder, and put in any .rtf or .otf font, and they will show up here.",
                    wrap=0,
                )
                dpg.add_text(
                    "Whatever font you choose will be applied to the whole application. Sometimes it's best to close and reopen the application after setting a font, in case things get weird.",
                    wrap=0,
                )
                dpg.add_combo(
                ...
                with dpg.group(horizontal=True):
                    dpg.add_checkbox(
                        label="Autosize?", tag="chkbxCFPAutosize", default_value=True
                    )
                    dpg.add_button(
                        label="Change Font", callback=lambda: self.btnBuild()
                    )
            ...
            with dpg.group(horizontal=True):
                ...
                dpg.add_button(
                    label="Reset to Default (Your Saves Won't Be Changed)",
                    callback=lambda: dpg.bind_font("DEFAULT")
                    and dpg.set_global_font_scale(1.0),
                )
